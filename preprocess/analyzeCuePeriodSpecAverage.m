function analyzeCuePeriodSpecAverage(Dataset, options, label)
...
for n = 1:MaxN
    ...
    stat{n} = analyzeCuePeriodSpecAverageForSubject(Dataset{n}, options, label);
end

freq_cue_roi = stat{1}.freq_cue_roi;
wm_hit = nan(length(freq_cue_roi), MaxN);
wm_cr = nan(length(freq_cue_roi), MaxN);
wm_oh = nan(length(freq_cue_roi), MaxN);
...
errorbar(freq_cue_roi, mean_wm_hit, sem_wm_hit, '-', 'Color', co(1,:));
plt1 = plot(freq_cue_roi, mean_wm_hit, '-', 'Color', co(1,:));
...
errorbar(freq_cue_roi, mean_wm_cr, sem_wm_cr, '-', 'Color', co(2,:));
plt2 = plot(freq_cue_roi, mean_wm_cr, '-', 'Color', co(2,:));
xlim([min(freq_cue_roi),max(freq_cue_roi)]);
ylim([Y_MIN, Y_MAX]);
xlabel('Frequency (Hz)');
...
if sum(~isnan(sum(wm_oh,1))) > 0
    ...
    errorbar(freq_cue_roi, mean_wm_oh, sem_wm_oh, '-', 'Color', co(3,:));
    plt3 = plot(freq_cue_roi, mean_wm_oh, '-', 'Color', co(3,:));
